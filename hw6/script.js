...
// Problem 6: ADC Sampling Rate Statements (Improved)
function initAdcSamplingRateAnimation() {
    ...
    optionsBits.innerHTML = `
        <div class="sine-wave-container">
            <div class="original-wave" id="original-wave">
                <div class="wave-label original">Original Signal (1 Hz)</div>
            </div>
            <div class="sample-points" id="sample-points">
                <div class="wave-label samples">Sample Points</div>
            </div>
            <div class="reconstructed-wave" id="reconstructed-wave">
                <div class="wave-label reconstructed">Reconstructed Signal</div>
            </div>
            <div class="sampling-rate-label" id="sampling-rate-label"></div>
        </div>
    ...
    function animate() {
        ...
        setTimeout(() => {
            samplingRateLabel.textContent = 'Sampling Rate: 1 Hz (Undersampling)';
            samplePoints.classList.add('undersampled');
            reconstructedWave.classList.add('aliased');
            addLabel(optionsBits, 'Sampling at 1 Hz (less than 2x signal frequency)', 0);
            addLabel(optionsBits, 'Aliasing: Reconstructed signal is distorted', 0, '-60px'); // Added offset for label
        }, 500);

        setTimeout(() => {
            samplingRateLabel.textContent = 'Sampling Rate: 2 Hz (Nyquist Rate)';
            ...
            addLabel(optionsBits, 'Sampling at 2 Hz (at least 2x signal frequency)', 0);
            addLabel(optionsBits, 'Nyquist Rate: Signal is accurately reconstructed', 0, '-60px'); // Added offset for label
    ...
    function addLabel(container, text, leftOffset, topOffset = '-30px') { // Added topOffset parameter
        const existingLabel = container.querySelector('.bit-label');
        if (existingLabel) existingLabel.remove();
        const label = document.createElement('div');
        label.className = 'bit-label';
        label.textContent = text;
        label.style.top = topOffset; // Use topOffset here
